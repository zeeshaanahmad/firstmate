...
- An adapter that is not verified for this task's kind is refused **before** the running agent is stopped, not after.
  Relaunching a secondmate onto a crewmate/scout-only adapter ([configuration.md](configuration.md#harness-support) owns which ones) refuses while its agent is still up rather than leaving that secondmate with no agent when the launch owner refuses.
