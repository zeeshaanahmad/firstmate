...
It classifies recognized Claude, Codex, OpenCode, Pi, pi-signed, Grok, Kimi, Cursor, Gemini, Muse, Rovo, and AGY process identities as `alive`, common shells as `dead`, an authoritatively absent window as `missing`, unreadable state as `unreadable`, and every other process as `ambiguous`.
...
Gemini ships as a node bundle whose `comm` and argv[0] both read as the interpreter, so its identity is read from the full foreground command line instead; [runtime-backends.md](verification/runtime-backends.md#gemini) owns that evidence.
...
That identity-gated exception preserves the strict container-proof rule for every other pane, so a modal dialog or a mid-redraw pane reads `unknown` rather than `empty`; a dead shell is not covered by this rule, as the sentences below explain.
