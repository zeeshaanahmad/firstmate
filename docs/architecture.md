...
Attribution reads the run object's own fields rather than the repeated `branch_sync` ones, and accepts a run head this worktree cannot resolve while the run is in flight, because the pipeline commits its fixes in its own gate repository and does not push them until the push step; a terminal run still has to prove its head (`bin/fm-nm-run-lib.sh` owns the rule and why each direction is safe).
...
A secondmate recovers that verdict without any liveness read at all, because its endpoint liveness is deliberately never read and so can never supply the dead confirmation an ordinary crew can; without that case a mate's captain hold, which has no current-state mapping, would be silenced rather than taking the bounded re-surface cadence, and a forgotten hold would rot invisibly.
A live parked crew still surfaces once per pause declaration - it may be sitting at an interactive gate its `paused:` line does not describe - anchored on the status file rather than pane content so a repainting idle pane cannot re-arm it; later sightings of the same declaration fall back to the bounded pause cadence.
...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review: it attributes a no-mistakes run, active or terminal, only when it matches the crew's branch and the code identity that run's own state calls for, then keeps that run-step authoritative even if the pane has closed.
That run object is the only source carrying gate detail, so a run rejected there does not degrade to a coarser verdict but to the pane, where a crew parked at a gate reads as busy or as nothing at all - which is why an in-flight run is not asked to prove a head that lives only in the pipeline's gate repository.
A parked verdict names the gate and which side must act: an ask-user finding is firstmate's decision under the authority contract, and any other gate is the worker's to drive.
`bin/fm-nm-run-lib.sh` owns the head-binding rule and the script header owns the rest.
...
That envelope is marked at both ends, because a pane can drop the head of a typed message and an away-mode receiver reads an unmarked message as the captain returning; the digest is also cut to a measured single-send delivery bound before it is typed, so an oversized escalation arrives marked and pointed at its durable full text, or is refused into the wedge alarm, rather than arriving as a headless fragment.
...
PR-based task merges go through `bin/fm-pr-merge.sh`, which records `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh`, runs the guards below, then calls the forge CLI.
...
Three guards run on both forges, because each reads the request's base branch and head from whichever forge hosts it and then compares them with git.
Immediately before the merge call, the merge-time static guard computes the exact merge result against the current base-branch tip and refuses a request whose combination with that branch is red; [docs/merge-time-static-guard.md](merge-time-static-guard.md) owns the guard and its red-default-branch detection counterpart.
A second, independent guard runs first and refuses squash, rebase, or the squash default for a request whose head carries commits an upstream remote also has and the base branch does not, because those methods rewrite that shared history out of the base branch's ancestry; [`bin/fm-pr-merge.sh`](../bin/fm-pr-merge.sh)'s header owns it, `--merge` is the remedy it names on GitHub rather than an override flag, and on GitLab no argument can prove a merge commit because the project's own merge method applies, so such a merge request is refused there and landed in GitLab instead.
A third guard is required for an upstream sync batch: it resolves the waypoint from `--require-ancestor` or the task's recorded `waypoint=` meta, refuses if the two are present and disagree, and otherwise refuses the merge unless that waypoint is an ancestor of the request head - [`bin/fm-pr-merge.sh`](../bin/fm-pr-merge.sh)'s header owns the mechanics, and the [`upstream-sync-batches` skill](../.agents/skills/upstream-sync-batches/SKILL.md) owns the batch procedure it protects.
