...
`bin/fm-liveness-lib.sh` owns that question and answers from a registered `state/<id>.liveness.sh` source or, with no declaration at all, from the `last_activity` of a non-terminal `no-mistakes` run attributed to that crew's own branch.
