...
PR-based task merges go through `bin/fm-pr-merge.sh`, which records `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh`, runs the merge-time static guard, then calls `gh-axi pr merge`.
...
Immediately before the merge call, the guard computes the exact merge result against the current default-branch tip and refuses a PR whose combination with current main is red; [docs/merge-time-static-guard.md](merge-time-static-guard.md) owns the guard and its red-default-branch detection counterpart.
A second, independent guard runs first and refuses squash, rebase, or the squash default for a PR whose head carries commits an upstream remote also has and the base branch does not, because those methods rewrite that shared history out of the base branch's ancestry; [`bin/fm-pr-merge.sh`](../bin/fm-pr-merge.sh)'s header owns it, and `--merge` is the remedy it names rather than an override flag.
A third guard is required for an upstream sync batch: it resolves the waypoint from `--require-ancestor` or the task's recorded `waypoint=` meta, refuses if the two are present and disagree, and otherwise refuses the merge unless that waypoint is an ancestor of the PR head - [`bin/fm-pr-merge.sh`](../bin/fm-pr-merge.sh)'s header owns the mechanics, and the [`upstream-sync-batches` skill](../.agents/skills/upstream-sync-batches/SKILL.md) owns the batch procedure it protects.
