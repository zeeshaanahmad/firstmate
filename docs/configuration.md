...
A profile `quota_scope` explicitly declares one quota-axi scope whose row also binds that candidate; it serves a provider that meters per tier under scope names the resolver's `all_models`, `all_products`, `model:<bare>`, and `product:<bare>` matching never reaches.
...
Antigravity meters per tier with its own clock, so an `agy` profile declares its tier, for example `{ "harness": "agy", "model": "claude-opus-4-6-thinking", "quota_scope": "claude_gpt" }` for the Claude/GPT tier and `{ "harness": "agy", "model": "gemini-3.8-flash-high", "quota_scope": "gemini" }` for the Gemini tier.
...
Malformed resolver-only declarations receive the same diagnostic only while typed resolution is active, as stated with the field semantics above.
...
Everything after the answer runs in code: the confidence floor, the matched rule's `approval` and `floor`, each candidate's `provider`, `quota_scope`, and `floor`, every applicable account-wide, model/product, and declared-scope row from one `quota-axi --json` snapshot, and the numeric `spendPriority` argmax over candidates using each candidate's limiting row.
