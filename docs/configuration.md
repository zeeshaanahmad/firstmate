...
There is no fallback target: with none of those present, firstmate is not running inside a supported pane at all, and both the shared entry path every away or quiet posture takes (`bin/fm-afk-launch.sh`) and the daemon refuse with that reason rather than guessing a pane.
The daemon additionally requires the identified pane to be running a live verified harness, proven from the pane's foreground processes rather than from what it renders, before it starts and again before every digest.
That second, non-rendered proof is why a pane whose agent has exited to an ordinary shell is refused even when the shell's prompt happens to render exactly like an empty agent composer.
Selecting any other supervisor backend, including `zellij`, `orca`, or `cmux`, refuses at daemon startup instead of trying tmux injection primitives against a non-tmux pane.

## Away-mode wedge alarm channels (config/wedge-alarm)

When away-mode injection wedges past `FM_MAX_DEFER_SECS`, the sub-supervisor raises a loud, rate-limited alarm; it raises the same alarm, unconditionally, when the daemon refuses to start because no supervisor pane could be identified or proven to hold a live agent.
Beyond the durable `state/.subsuper-inject-wedged` marker and, for a delivery wedge, the tmux status-line flash, it attempts a configured backend-independent active alert that can reach the captain even when every pane and its backend status-line is unreadable.
...
An absent file means `auto`, i.e. default-on on macOS: the alarm exists precisely so a wedged or unstartable away-mode primary is never silent, and the delivery-wedge case fires at most once per max-defer window after a genuine wedge.
...
When `config/crew-dispatch.json` exists, crewmate and scout spawns require an explicit resolved harness, model, and effort instead of automatically falling back to `config/crew-harness` and the session's own model and effort.
...
When the file exists, `fm-spawn.sh` enforces that contract by refusing crewmate and scout spawns that lack an explicit harness (`--harness`, a positional adapter, or a raw launch command), an explicit `--model`, or an explicit `--effort`; each axis is checked independently, so passing one does not excuse the others.
Batch spawns satisfy the same requirement with a shared `--harness`, `--model`, and `--effort`.
...
While the file remains present, no crewmate or scout spawn may proceed without an explicit resolved harness, model, and effort; malformed configuration must be reported and corrected rather than selected around.
...
FM_MERGE_GUARD=         # fm-pr-merge.sh's merge-time static guard posture: unset guards fully, off skips it and records merge_guard=off, allow-timeout still refuses red but merges past a check that could not finish; any other value is refused (docs/merge-time-static-guard.md)
FM_STATIC_CHECK_TIMEOUT=900  # seconds allowed for one whole merge-time check run; a hang bound, not a performance target, so raise it for a project whose own gate is slower rather than letting a green check be killed (docs/merge-time-static-guard.md)
...
FM_CREW_STATE_NM_TIMEOUT=10   # seconds allowed per no-mistakes query inside fm-crew-state.sh; hitting it reports unknown, never a state guessed from the status log or asserted from the run record alone
...
FM_LIVENESS_NM_TIMEOUT=10          # seconds the bounded `no-mistakes axi status` activity read may take before it is read as no answer
...
FM_INJECT_MAX_BYTES=800            # max encoded bytes the daemon types in one send; a larger digest is cut from the tail or refused to the wedge alarm rather than delivered as a fragment (docs/verification/supervision.md "Away-mode delivery bound")
