...
There is no fallback target: with none of those present, firstmate is not running inside a supported pane at all, and both `/afk` entry and the daemon refuse with that reason rather than guessing a pane.
