...
When a matched rule or default is a profile array, load `quota-array-dispatch` for the TOON-first spendPriority candidate choice after this skill establishes harness and model/provider facts.
...
That gate is Cursor's own structural process identity from `bin/fm-cursor-lib.sh`, never the verdict alone, so the strict blank-cursor-row posture stays in force for every other harness; that posture governs pane shape only and does not by itself rule out a dead shell, which can render the same affirmative `empty` verdict (`docs/herdr-backend.md` "Composer and injection safety").
