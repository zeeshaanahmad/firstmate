...
- The daemon's injection envelope is marked at both ends, so a message that begins mid-sentence and ends with the U+2063 `FIRSTMATE_OP_END:` terminator is a daemon escalation whose front was cut in delivery, never the captain talking; stay away and recover the full text from the daemon log the digest points at.
