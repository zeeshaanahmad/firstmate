...
family_for_basename() {
  case "$1" in
    ...
    fm-daemon.test.sh|fm-guard-stale-banner.test.sh|fm-liveness-source.test.sh|\
    ...
    fm-review-diff.test.sh|fm-teardown.test.sh|fm-x-mode.test.sh)
      printf '%s\n' pr-forge
      ;;
    fm-afk-contract.test.sh|fm-afk-inject-e2e.test.sh|fm-afk-return.test.sh|fm-afk-shell-pane-refusal.test.sh)
      printf '%s\n' afk
      ;;
...
tests/fm-claude-attribution.test.sh 9500
...
tests/fm-watcher-signal-safety.test.sh 36000
EOF
}
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-liveness*)
      ...
      # and teardown removes its per-task artifacts.
      printf '%s\n' watcher-wake-lock
      printf '%s\n' pr-forge
      ;;
    bin/fm-check-lib.sh)
      # The shared task-scoped-executable byte binding, used by both the custom
      # watcher check (pr-forge) and the liveness source (watcher-wake-lock).
      printf '%s\n' pr-forge
      printf '%s\n' watcher-wake-lock
      ;;
