...
#
# Usage: fm-pr-merge.sh <task-id> <pr-url> [--attended-override] [--allow-red <check-name>] [--require-ancestor <sha>] [-- <extra forge merge args>]
#
...
# The merge guards run before the per-task control lock below is taken. A
# check can run for minutes, and the watcher's merge poll and every other
# per-task control operation wait on that lock, so holding it here would
# stall them for the whole check.
