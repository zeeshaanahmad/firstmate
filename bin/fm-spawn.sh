...
#   The literal value "default" means the caller deliberately wants the
#   harness's own default for that axis; it is the one way to express "omit this
#   flag" once config/crew-dispatch.json makes the flag required (see the harness
#   paragraph below), and it emits no --model/--effort flag at launch,
#   identically to leaving the flag off when no dispatch profile file is active.
...
case "$EFFORT" in
  ...
  *) echo "error: --effort must be one of default, low, medium, high, xhigh, max, ultra" >&2; exit 1 ;;
esac
