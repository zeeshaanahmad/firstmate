...
launch_cmd() {  # <name>
  case "$1" in
    claude) printf '%s' 'CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 claude --dangerously-skip-permissions --settings '\''{"attribution":{"commit":"","pr":"","sessionUrl":false},"feedbackDrafts":"off"}'\''' ;;
