...
#
# Use this only where a completed pass is genuinely the strongest available
# proof. Where the assertion depends on a SPECIFIC marker or counter, a
# completed cycle is only a proxy for it: wait on that artifact instead, with
# wait_absorbed or wait_stale_passes from tests/wake-helpers.sh.
...
# The wait_for_exit budget in this file is 100 ticks (10s), not because any
...
# The declared-pause regression cases below instead pass $FM_TEST_WAIT_TICKS,
# the shared env-tunable ceiling from tests/wake-helpers.sh, because those cases
# spawn a watcher per round and were the ones observed reaping mid-startup under
# heavy load.
wait_numeric_file() {
...
test_live_declared_pause_gate_surfaces_once_per_declaration() {
  ...
  for round in already-surfaced new-declaration; do
    ...
    : > "$state/.paused-$key"
    back=$(( $(date +%s) - 300 ))
    # The re-surface throttle reads this marker's CONTENT against the
    # declaration's own scope, so a round is decided by which scope the marker
    # names; each round also ages the pair consistently so the fixture cannot
    # accidentally depend on marker age.
...
# The recorded pause flag is authoritative on its own, and reading agent liveness
# there is a PROMOTION of an inconclusive verdict, never a demotion of the flag.
# The A/B is on the one input that separates them: a lane that already declared a
# pause and already had its declaration surfaced, whose authoritative crew state
# has since fallen back to unknown while its agent is still very much alive. A
# live agent is the normal shape of a parked crew, so vetoing the recorded flag on
# liveness surfaces that lane as "stopped responding" on every new pane hash. This
# case enters first sight of a NEW stale hash - the only path where the pause flag
# and its fresh recheck decide the verdict on their own - and asserts the lane is
# absorbed with its recheck marker intact.
test_live_pause_flag_absorbs_when_authoritative_state_falls_back() {
  local dir state fakebin out capture_file statusf window key pane pane_hash pid back queued
  dir=$(make_case paused-flag-live-fallback); state="$dir/state"; fakebin="$dir/fakebin"
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; statusf="$state/parked.status"
  window="test:fm-parked-fallback"
  key=$(printf '%s' "$window" | tr ':/.' '___')
  pane='idle at an empty composer'
  ...
  wait_stale_passes "$state" "$key" "$pid" 1 3 \
    || { reap "$pid"; fail "a parked lane whose crew state fell back was not absorbed: $(cat "$out")"; }
  ...
  [ ! -s "$out" ] \
    || fail "a parked lane with a live agent surfaced as stale on a fallen-back crew state: $(cat "$out")"
  queued=$(awk -F '\t' -v w="$window" '$3 == "stale" && $4 == w { n++ } END { print n + 0 }' \
    "$state/.wake-queue" 2>/dev/null || echo 0)
  [ "$queued" -eq 0 ] \
    || fail "a parked lane with a live agent queued $queued stale wake(s) on a fallen-back crew state"
  pass "a recorded declared pause absorbs on a live agent when authoritative crew state falls back"
}
...
test_secondmate_unpause_clears_pause_tracking() {
  ...
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  # Wait on ALL THREE markers the assertions below read, not just the pause one:
  # clear_pause_tracking removes .paused-<key> first and the stale/wedge pair
  # after, so a predicate naming only the pause marker can win mid-clear and
  # leave the later assertions reading state the watcher was still removing.
  # shellcheck disable=SC2016 # deliberately deferred: wait_absorbed evals this, not this shell
  wait_absorbed "$pid" '[ ! -e "$state/.paused-$key" ] && [ ! -e "$state/.stale-$key" ] && [ ! -e "$state/.wedge-escalations-$key" ]' \
...
test_captain_held_never_rechecked_while_away_record_exists() {
  ...
  reap "$pid"
  ack_stopped_cycle_if_any "$state" || fail "could not acknowledge the intentional phase-A stop"
