...
test_reassigned_pool_slot_finishes_own_cleanup_without_touching_the_slot() {
  ...
  # bin/fm-teardown.sh refuses a non-forced ship cleanup without the completion
  # report the ship brief requires (tests/fm-teardown.test.sh owns that
  # contract); this case is about slot ownership, so it seeds a placeholder.
  mkdir -p "$dir/home/data/$id"
  printf '1. SUMMARY - fixture.\n' > "$dir/home/data/$id/completion-report.md"
  claim_pool_slot "$dir" "$other" "$dir/other-home"
  ( cd "$dir/worktree" && exec sleep 30 ) &
  worker=$!
